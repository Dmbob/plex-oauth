...
export class PlexOauth {
  ...
  public requestHostedLoginURL(): Promise<[string, number]> {
    return this.authPin
      ...
      .then((codeResponse) => {
        return [
          `${LinkHelper.PLEX_AUTH_BASE_PATH}#?code=${
            codeResponse.code
          }&context[device][product]=${
            this.clientInfo.product
          }&context[device][device]=${this.clientInfo.device}&clientID=${
            codeResponse.clientIdentifier
          }&forwardUrl=${
            this.clientInfo.forwardUrl ? this.clientInfo.forwardUrl : ""
          }`,
          codeResponse.id,
        ] as [string, number];
      })
  ...
  public checkForAuthToken(
    ...
    if (!pinId) {
      throw new Error(
        "Pin Id is not set - Unable to poll for auth token without id"
      );
    }
